...
import { ShoppingCart, Search, X, LogOut, User } from "lucide-react";
        ...
        <div className="px-4 md:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            <button
              ...
              className="md:hidden text-text-muted hover:text-accent-gold transition-colors cursor-pointer"
            >
              {mobileSearchOpen ? (
                <X className="w-5 h-5" />
              ) : (
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <span className="text-text-secondary text-xs font-mono hidden md:block">
                  {user.name}
                </span>
                <button
                  onClick={logout}
